...
public class SendFragmentedEcho {
  ...
  private static final int MTU = Integer.getInteger(MTU_KEY, 1400); // [bytes]
  ...
  public static void main(String[] args) throws PcapNativeException {
    ...
    try {
      handle.setFilter(
        "icmp", BpfCompileMode.OPTIMIZE);

      PacketListener listener = packet->{
        try {
          EthernetPacket.EthernetHeader header = EthernetPacket.newPacket(packet.getRawData(),0,packet.length()).getHeader();
          System.out.println("Type: "+header.getType().name());
          System.out.println("Src: "+header.getSrcAddr().toString());
          System.out.println("Dst: "+header.getDstAddr().toString());
...
//      udpBuilder.srcAddr(InetAddress.getByName(strSrcIpAddress))
//        .dstAddr(InetAddress.getByName(strDstIpAddress))
//        .srcPort(new UdpPort((short)9999,"me"))
//        .dstPort(new UdpPort((short)7091,"me"))
//        .payloadBuilder(new UnknownPacket.Builder().rawData(echoData))
