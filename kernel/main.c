...
static void ksocket_start(void)
{
		...
		ikcpcb *kcp = ikcp_create(0x114514,kthread);
		kcp->output = udp_output;
		printk("conv:%ld",0x114514);
        ...
        for (;;)
        {
				...
				printk("time: %ld",mytime.tv_nsec);
				ikcp_update(kcp,mytime.tv_nsec);
                ...
                else 
                {
						...
						while(1){
	                        ...
	                        /* data processing */
	                        printk("\n data: %s\n", buf);
							ikcp_send(kcp,"testing...",strlen("testing..."));
						}

                        /* sending */
                        //memset(buf, 0, bufsize+1);
                        //strcat(buf, "testing...");
						//ksocket_send(kthread->sock, kthread->msghdr->msg_name, buf, strlen(buf));
                }
        }
		ikcp_release(kcp);
close_and_out:
...
int udp_output(const unsigned char *buf,int len,ikcpcb *kcp,void *context){
	struct kthread_t *kthread = (struct kthread_t *)context;
	return ksocket_send(kthread->sock,kthread->msghdr->msg_name,buf,len);
}
int ksocket_send(struct socket *sock, struct sockaddr_in *addr, unsigned char *buf, int len)
{
        struct msghdr msg;
